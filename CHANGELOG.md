...
- **`windows` and `qemu` smoke steps in `test-upstream-usermodule.yml`**, closing
  ...
  against the collected `MPS2_AN385` `firmware.elf` with `-serial pty`, and runs
  `mpremote ... run` over the printed pty device. Both paths (a passing script
  and a deliberately broken one) were verified live against real cross-compiled
  binaries before being wired into CI — `rp2`'s own `.uf2` still has none, for
  the same hardware/emulator reason record 0069 gives. Record 0080.
