# `unix` build image for the **manylinux_2_28_s390x** cell -- one native
# image per (architecture, libc floor), record 0043.
#
# `FROM` is pypa's own `manylinux_2_28_s390x`, digest-pinned from
...
# the gcc that targets s390x, the whole userland -- comes from there.
...
# rather than a label.
#
# **This is a `linux/s390x` image**, built and published for its own
...
# `exec format error`.
#
# Build: docker buildx build --platform=linux/s390x \
#          -t manylinux_2_28_s390x \
#          -f docker/manylinux_2_28_s390x.Dockerfile .
# Use:   CIBMP_UNIX_MANYLINUX_2_28_S390X_DOCKER_IMAGE=manylinux_2_28_s390x cibuildmp ...
#
...
FROM quay.io/pypa/manylinux_2_28_s390x@sha256:2ff3207ba86c8fef999eb2e1139a612501501493f3fd47bbc8a71199a29e9de4
...
# libffi`, and inside a stock `manylinux_2_28_s390x` that command fails
