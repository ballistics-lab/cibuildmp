...
means `fetch-micropython` or `clone-micropython` (this repo's own) must
have already run in the same job; a composite action step can't set an env
var that steps *before* it will see, only ones after.
...
jobs:
  build:
    ...
    steps:
      ...
      - uses: ballistics-lab/cibuildmp/.github/actions/fetch-micropython@v0.7.0
        with:
          mpy_tag: v1.28.0

      - uses: ballistics-lab/cibuildmp/.github/actions/build-natmod@v0.7.0
        with:
          arch: ${{ matrix.arch }}
