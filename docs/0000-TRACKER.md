...
- [ ] [0056] build upstream MicroPython through the usermod path with no user C module | **decided**: `no-user-c-modules = true`, mutually exclusive with `user-c-modules` -- both given is a load-time error, not a precedence rule, since either winner silently ignores an explicitly written key ([0048]'s own lesson). The check must test *explicitly set*, not *has a value*: `user-c-modules` defaults to `"."`, so it reads through `opt("user-c-modules")` with no `default=`, which is `None` only when unset at every layer. Upstream needs nothing -- empty is a clean no-op on both sides (`ifneq` in `py/py.mk`, `ifdef` guarding the CMake ports' forward). `manifest` needs no flag (its default is already `""`), and a stock build *with* a frozen manifest is legitimate -- which exposes that the manifest reaches the container only via the `user-c-modules` mount of `"."`, so the mount list has to be rebuilt, not just shortened
