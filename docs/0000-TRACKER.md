...
| **behaves like cibuildwheel** | **[0051] (all points, phases E-I) closed the identifier/cascade gap it set out to close -- but doing that surfaced a larger one: cibuildmp's own axes were never a genuine matrix the way cibuildwheel's are, argued in [0052] as a deliberate divergence (tree-addressed config, not a flat selector matrix). [0052]'s own record carries a full implementation-plan addendum (independent Track A items, a sequential Track B for the tree mechanism itself). Landed so far: all of Track A (A1 arch_flags dedup, A2 natmod's `micropython`/`mpy-abi` config keys removed and identifier drops `-natmod-`, A3 `{name}-{version}-{identifier}` artifact filenames for both families, A4 `arch_flags = 0` documented as a compatibility class, A5 pre-build reachability audit for both families) and Track C's Phase C1 (natmod resolves tags/archs against `resources/build-platforms.toml`, hard errors instead of silent fallbacks). **Track B's own B4.1-B4.3 (the tree-walk mechanism, esp32 boards as tree nodes, tree-path `select` matching) were built, then reverted the same day** -- live-checked, `usermod`'s own identifier already carries `{port}` as a literal, stable substring (`*-esp32-*` already reaches every esp32 board via plain glob), so tree-path matching added no reachable case flat identifier globbing did not already have; confirmed against real `cibuildwheel/options.py` that upstream's own actual gap is different -- `build`/`skip` genuinely are settable per-platform there (`[tool.cibuildwheel.linux] build = ...`), which cibuildmp's own [0048]-era `GENERIC_KEYS` mechanism has always forbidden. See [0052]'s own latest addendum for the full argument. Only esp32 board-name validation (against `resources/build-platforms.toml`) survived the revert, reapplied onto the flat `boards = [...]` list unchanged. **Per-platform `build`/`skip` is now landed** -- `[natmod]`/`[usermod]`/`[<port>]` can all set their own `build`/`skip` now, more-specific-wins, matching upstream's own `[tool.cibuildwheel.<platform>]` shape; `build`/`skip` moved out of `GENERIC_KEYS` into each side's own schema (`NATMOD_SCHEMA`, a new `USERMOD_PLATFORM_KEYS`, kept separate from the `[[overrides]]` schema to avoid a real circularity). `UsermodOptions.targets()` resolves build/skip per active port now, reassembled in original order; A5's own `check_reachable()` checks each port's own value too, scoped to that port, only when the port genuinely sets one of its own (a family-wide pattern falling through to a port unmodified is not re-checked against that port's own narrower domain -- a real false positive caught live while writing the first test against it). **The cross-family `[[overrides]]` reachability gap is now fixed, one direction**: usermod's own `check_reachable()` widens its override-reachability domain with natmod's own `all_targets()` (computed from the same raw config, via the existing `preread` mechanism, `except NatmodConfigError: return []` for safety), since usermod already imports from natmod one-way -- fixes the exact live bug reported (`cibuildmp --dry-run --platform unix` against this repo's own real `cibuildmp.toml`, rejecting the natmod-only `select = "*-armv7emsp"` override as unreachable). Natmod's own side is *not* fixed (it must never import usermod back) -- a natmod-only invocation flagged by a usermod-only override is still possible, tracked as the residual half of the same task. **natmod's own `Target.identifier` now matches a real row instead of rebuilding one** -- raised live by the user against real CLI output: the identifier string was correct but computed by an f-string never checked against `build-platforms.toml`'s own (stale, pre-A2) `identifier` field. Confirmed against real `cibuildwheel/platforms/linux.py`: `PythonConfiguration.identifier` is a literal field read off a row, never computed -- natmod's 205 rows regenerated, `Target.identifier` now looks its base string up and appends only the config-driven `arch_flags` suffix. The user's own further correction removed `archs_available_for(tag)`/`_TAG_ARCHS`/`natmod_targets()` entirely: narrowing now runs *after* `select()`'s own glob match against real rows (`natmod_all_targets()`), not via a purpose-built availability table computed before selection. **A same-day, live-caught reversal of the first version of that fix**: the initial regeneration dropped tag from the identifier (`mpy{mpy}-{arch}`) and added `collapse_newest_tag()` to resolve the resulting ambiguity -- wrong, per the user's own direct correction and concrete evidence (`mpy5-x86` alone collapses seven real, distinct rows across tags `v1.12`-`v1.18`). Tag is back in the identifier (`identifier_format = "mpy{mpy}-{tag}-{arch}"`, e.g. `mpy6.3-v1.30.0-preview-armv7emsp`), `collapse_newest_tag()` deleted outright (every `(tag, arch)` row is now unique, nothing left to collapse), replaced by `selector_names_a_tag()` (does `build` literally name a tag-shaped substring?) + `narrow_to_newest_tag()` (only applied when it does not) -- verified live via both branches (`--dry-run --platform natmod` narrows to the newest tag with no selector; `--only "mpy6.3-v1.23.0-x64"` trusts an explicit historical tag as-is). **The same live conversation also simplified `[[overrides]]`'s own TOML syntax**: an array of `{select = "glob", ...}` tables becomes one `[override]` table keyed directly by its own glob (`[override."*-armv7emsp"]`, no `select` field inside) -- a deliberate divergence from upstream's own `[[tool.cibuildwheel.overrides]]` shape, argued in [0052]'s own latest addendum; declaration order (which `tomllib` already preserves) is precedence, exactly as file order already was, so no new depth/specificity mechanism was needed or built. Full addenda for both: [0052]'s own two newest entries. **`archs` is gone as a natmod config key entirely, the same day** -- it filtered candidate rows by `.arch` alone, before `build`/`skip` ever ran, duplicating exactly what a `build`/`skip` glob over the identifier already expresses (`build = "*-x64"`); raised live against the root `cibuildmp.toml`'s own `[natmod] archs = [...]` line, which turned out to list every real natmod arch verbatim -- the config's own default, spelled out in full, doing nothing at all. `Options.archs`, `NATMOD_SCHEMA`'s own `archs` entry and `validate_archs_recognized()` deleted outright; `--archs` on the CLI now raises a specific, actionable error for natmod (there is no safe general way to translate an arbitrary CLI arch list into a `build`/`skip` glob rewrite) while staying unaffected for usermod ports. See [0052]'s own newest addendum for the full argument, the fixture ripple, and two live-caught corrections to the immediately preceding addendum (unix's real identifier carries no `-unix-` substring at all, `manylinux`/`musllinux` is the actual marker; every platform has its own marker, unix is not a special case). **`platform_tables` is now gone from the cascade entirely, the same day** -- the "don't duplicate what a glob already does" argument extended one tier further: a per-platform TOML table (`[unix] user-c-modules = "..."`) was always exactly `[override."*-{manylinux,musllinux}*"] user-c-modules = "..."` restated, since every platform's own real identifier already carries a marker a glob/`[override]` selector can address directly. Retracted: the "per-platform build/skip" addendum and Phase F's own per-platform option-key wiring, both landing back at global-only. Deliberately *not* retracted: `[usermod]`'s own family tier (kept at the user's own explicit prior insistence -- a real, still-load-bearing distinction for `extra-make-args`, shared by name between natmod and usermod) and the per-platform *environment* override (`CIBMP_BUILD_NATMOD`, a real, distinct capability untouched by the TOML argument). `natmod/options.py`'s `NATMOD_SCHEMA` is now empty; `usermod/options.py`'s own per-port `SCHEMAS[port]` shrinks to just that port's own axis key. A real, separately-tracked gap surfaced in the process: `build`/`skip` being genuinely global now means a pattern meant for one active platform fails the *other* active platform's own reachability check (natmod cannot widen with usermod's own identifiers without importing it back) -- the same shape as the still-open natmod-side half of the cross-family `[override]` reachability gap, now also reachable through `build`/`skip`; `CIBMP_BUILD_NATMOD` is today's real workaround, proven live in the multi-platform test fixtures. See [0052]'s own newest addendum for the full argument. Still open: Track C's Phase C2 (usermod's own identical problem, more literal -- zero rows consulted at all today, plus a real `windows`/`webassembly` data-format reconciliation this same investigation surfaced), A6's own docker-image-resolver redesign, that residual natmod-side half of the cross-family reachability gap (now spanning build/skip too), and a floated-but-undecided further simplification (defaults themselves as a lowest-priority built-in `[override."*"]` entry, collapsing the cascade's own `default=`/`family_table` layering into pure glob/override matching) -- raised in the same conversation, not yet designed or implemented** |
...
- [x] [0051] one selector for both modes, and an identifier that names what a build is compatible with (epic, points 1-8, phased E-I) | landed 2026-08-26, one session: natmod's `mpy-abi` and usermod's `micropython` are both real lists (two ABIs/releases in one config no longer overwrite each other's output); `select()`/`matches()`/`enable`/`groups` unified in `cibuildmp/selector.py`; the config tree flattened -- `[unix]`/`[windows]`/`[qemu]`/`[webassembly]`/`[esp32]` are sibling top-level tables to `[natmod]`, table presence selects a platform, `--platform`/`CIBMP_PLATFORM` narrows a set of six rather than choosing a mode, and more than one platform builds in one invocation with no flag at all; one shared top-level `[[overrides]]` with `inherit = {key = "append"|"prepend"|"none"}`, validated loosely (any platform) at parse time and strictly (the matched identifier's own platform) at build time; `natmod`/`usermod` physically merged into `cibuildmp/platforms/`, `cli.py`'s dispatch reached only through a `PLATFORM_FAMILY` registry (never hardcoded branches, so a future platform family costs one module + registry entries, zero `cli.py` changes) -- corrected mid-session after review showed the original two-branch design wouldn't generalize to zephyr ([0022]) or upstream's other real ports; README/`action.yml` reconciled. Superseded [0048]'s parse-time key partition with upstream's own cascade model (that record's own addendum). **The sixth addendum's own `module-dir`→`user-c-modules` rename also landed the same day**, together with a real third cascade tier (`default → global → family → platform → env → CLI`) and `[usermod]`'s own return as a non-selector family-defaults table, sibling to `[natmod]` -- chosen over a lighter code-only alternative at the user's own explicit insistence ("easier to document and maintain without collisions"); `usermod`'s own default for its renamed key changes from `"usermod"` to `"."` at the same time. `cli.py` still never names a family directly: every `PlatformModule` gained a third contract function, `validate_family_table()`, called generically over `PLATFORM_FAMILY`'s own deduplicated values -- a real regression against that invariant was introduced and caught within the same pass. Four independently-scoped follow-ons surfaced and deliberately deferred, not started: `[publish]`/`extra-files` split per family, vanilla/no-module usermod builds (the eighth addendum's own non-goal, since lifted), a formalized `build-platforms.toml` (turns out `resources/natmod.toml`'s own `[mpy-abi]` table already mostly is one), and a cross-family `FAMILY_SCHEMA` collision guard. Full account in the record's own ninth addendum. Precedes [0038] deliberately -- the identifier shape and config tree are now settled, so the three consuming repos migrate once
