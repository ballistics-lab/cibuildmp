...
- [ ] [0058] the image axis is the toolchain, not the port | **data half landed**: every port table in `build-platforms.toml` now names its image group beside `identifier_format`/`post_checkout` -- a scalar `image` for thirteen ports, an `images.<board>` map for `qemu` (three toolchains across its boards, closing [0052]'s own open "qemu per-board" item), `images.<arch>` for `[natmod]`, and fifteen identity keys for `unix` so `image_for()` keeps no special case. Corrects [0044]/[0052]'s sketch twice: the key is `arch`, **not** `cross` (`cross` is per-tag and absent exactly where it misleads -- `armv6m` through v1.18 would route to the host image), and the four groups are six (`natmod_host`, `ppc64le_linux`). Not started: no Dockerfile written, `pinned_docker_images.toml` still two tables, `image_for()` still branches per port
