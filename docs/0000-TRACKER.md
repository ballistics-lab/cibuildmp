...
| **behaves like cibuildwheel** | **[0051] (all points, phases E-I) closed the identifier/cascade gap it set out to close -- but doing that surfaced a larger one: cibuildmp's own axes were never a genuine matrix the way cibuildwheel's are, argued in [0052] as a deliberate divergence (tree-addressed config, not a flat selector matrix). [0052] is now the priority ahead of [0038] and every other config/identifier-touching epic ([0022], [0040]) -- same "don't migrate twice" reasoning that already sequenced [0051] before [0038], one level earlier. [0052]'s own record carries a full implementation-plan addendum (independent Track A items, a sequential Track B for the tree mechanism itself). Landed so far: all of Track A (A1 arch_flags dedup, A2 natmod's `micropython`/`mpy-abi` config keys removed and identifier drops `-natmod-`, A3 `{name}-{version}-{identifier}` artifact filenames for both families, A4 `arch_flags = 0` documented as a compatibility class, A5 pre-build reachability audit for both families) and Track C's Phase C1 (natmod resolves tags/archs against `resources/build-platforms.toml`, hard errors instead of silent fallbacks). Track B's own B0-B3 review pass is now accepted (tree-walk replaces the platform_tables tier; dotted TOML nesting, a deliberate reversal of [0051]'s own flat-sibling-table shape already flagged as pending there; `[[overrides]]` `select` gains tree-path matching alongside identifier matching, additive, `build`/`skip` unaffected; `**` not needed), one new question surfaced for B4 to answer (should `arch-flags` become a legal per-arch-node override key now that a tree node can address one arch unambiguously). B4.1 (data model) is landed too: `cibuildmp/options.py`'s cascade class is now one recursively-addressed `tree` instead of separate `family_table`/`platform_tables` fields -- natmod's own call sites unchanged, usermod's one call site now passes a 2-tuple path; the TOML file shape itself is untouched until B4.5. B4.2 (config loading) is landed: esp32's own `boards = [...]` config key is deleted -- `[esp32.BOARD_NAME]` tree-node presence selects a board now, validated against `resources/build-platforms.toml`'s own 57 known board names (loosely, not tag-gated -- that stays Track C Phase C2's own job). `usermod_targets()`/`UsermodTarget` needed no changes; only `UsermodOptions.load()`'s own axis computation changed. B4.3 (dual `[[overrides]]` matching) is landed too: `select` now matches by tree path (`"usermod.esp32.*"`) additively alongside identifier globs -- caught and fixed in the same phase, A5's own `check_reachable()` needed the same additive treatment for `[[overrides]]`'s own `select` (a correction to this record's own B0-B3 review, which had claimed no change was needed there). Found live, not fixed yet, tracked separately: a genuinely pre-existing bug (predates Track B) where `check_reachable()` checks a shared `[[overrides]]` entry against only its own family's identifiers, rejecting a config whose override was legitimately meant for the other family. Still open: Track B's own B4.4-B4.5 (`targets.py`'s own per-target option reads, and the actual file-shape migration), Track C's Phase C2 (usermod), and A6's own docker-image-resolver redesign** |
