...
- [x] [0050] does `xtensa-lx106` (ESP8266) earn its share of the natmod image |
      **kept, decided 2026-08-28.** Confirmed small: ~38MB compressed,
      smallest of the four toolchains by a wide margin (`riscv-none-elf`
      ~433MB, `arm-none-eabi` ~307MB, `xtensa-esp-elf` ~89MB) -- dropping it
      would lose one of ten supported natmod arches (real ESP8266 HW) for a
      size win the layer-reorder above already captured
