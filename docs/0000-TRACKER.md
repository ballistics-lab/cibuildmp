...
| **behaves like cibuildwheel** | **[0051] closed the identifier/cascade gap; [0052] then found and closed the larger one it surfaced -- cibuildmp's own config is fully tree-addressed now, not a selector matrix.** Landed: every per-platform table (`[natmod]`/`[unix]`/`[windows]`/`[qemu]`/`[webassembly]`/`[esp32]`) and every activation/keyword mechanism riding on it (`--platform`/`CIBMP_PLATFORM`, `--only`, `--archs auto`/`native`/`all`, `enable`/`GROUPS`) are retracted -- every platform is always in scope, `build`/`skip` glob-matching the real identifier (config, env, or `--build`/`--skip` on the CLI) is the only selection mechanism left, and zero-config now genuinely builds nothing. Usermod gets natmod's own real-row model (`all_usermod_targets()` reads every real `(port, tag, arch/board)` row directly, closing Track C's own Phase C2); the cross-family `[override]`/`build`/`skip` reachability gap (task #66) is closed both directions via `cli.py`'s own coordinator supplying `foreign_identifiers`; `narrow_to_newest_tag()` now prefers a stable release over a preview sharing an ABI. See [0052]'s own addenda for the full history. Still open, out of scope for that record: A6's own docker-image-resolver redesign, and the ten usermod ports `resources/build-platforms.toml` already has verified rows for but no real build driver (rp2, mimxrt, samd, stm32, psoc-edge, alif, esp8266, cc3200, renesas-ra, nrf) -- flagged as the genuinely larger remaining task, not a config-schema question.** |
