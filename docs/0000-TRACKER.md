...
- [ ] [0069] a real CI slice of [0054], not just its own scoping | `.github/workflows/test-upstream-usermodule.yml` builds upstream's own `examples/usercmodule/` on one Make port (`unix`) and one CMake port (`rp2`), resolving the pinned checkout via `sources.fetch_micropython()` in the workflow itself (no vendoring, no new config template). `examples/usercmodule/micropython.cmake` is this repo's own three-line shim adding the `subpackage` include() upstream's own aggregator omits. `esp32`/`windows`/`webassembly`/`qemu` and the `cppexample` skip question left to widen later
