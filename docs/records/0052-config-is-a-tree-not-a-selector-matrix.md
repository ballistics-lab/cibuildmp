...
## Addendum, 2026-08-27 — B4.2 landed: esp32 boards are tree nodes, `boards = [...]` deleted

Real, user-facing breaking change, the same class as A2's own `-natmod-` removal, landed
with the same care -- dedicated migration error, own tests, own addendum, not folded
silently into a mechanical pass.

**Scope decided precisely, before writing code, by re-reading B4.2's own text and B4.4's
own text together**: B4.4 ("targets.py changes") says `UsermodTarget` gains no new field
and target generation itself is untouched by Track B -- only *which config node
`build_options()` reads from* changes, and only once B4.4 itself lands. That meant B4.2's
own job is narrower than "wire boards all the way through": it is *config loading* only --
parsing, and validating that a written `[esp32.<name>]` node is well-formed and names a
real board -- not yet *reading* a board's own overrides into a real build (`build_options()`
still resolves `platform=("usermod", target.port)`, a 2-tuple, unchanged from B4.1; a
board's own `extra-make-args` override is parsed and stored in the tree correctly today,
but not yet applied to any build until B4.4 wires the read side). Flagged here explicitly so
it is not mistaken for an oversight.

**`usermod_targets()`/`UsermodTarget`/`_PORT_AXES` needed zero changes**, confirmed by
writing the design before the code: `axis_key("esp32")` still returns `"boards"` (the axis
*concept* is unchanged, only how its values are read from config), and the loader now
computes `axis_overrides["esp32"]` from which `[esp32.<name>]` sub-tables are present
rather than from a `boards = [...]` value -- the exact same shape `usermod_targets()`
already consumed, so target generation is untouched, exactly as B4.4's own text implies it
should be at this point.

**`TREE_ADDRESSED_AXIS_PORTS` (`usermod/targets.py`), currently `{"esp32"}` alone.**
`qemu` also has a real `"boards"` axis (`_PORT_AXES["qemu"]`) but is deliberately excluded
-- its own board set is small and closed (`VIRT_RV32`/`VIRT_RV64`/`MPS2_AN385`), not the
400+-row, independently-verified catalog `resources/build-platforms.toml` already has for
esp32; the gap this phase closes (validated existence) was never qemu's. `zephyr` ([0022],
not started) is the set's own second future member, not added speculatively ahead of
existing.

**Board name validation, live-verified against the real resource before writing the check**:
`build_platforms_data()["usermod"]["esp32"]["identifiers"]` has 442 rows, 57 distinct board
names (`board` field) across every tag it has walked -- confirmed directly, not assumed.
`KNOWN_ESP32_BOARDS` is the flat union of all 57, **not tag-gated** -- a board name that
exists for `v1.20.0` but not `v1.29.0` still passes this check today, since asking "does
this board exist for *these specific selected tags*" needs `all_targets()`'s own tag-gated
domain the way natmod's `archs_available_for(tag)` already has (Track C, Phase C1) --
usermod's own equivalent is Track C's own Phase C2, still not started, deliberately not
pulled forward into this phase. What this phase actually closes: before it, `boards =
[...]`'s own values were **never validated against anything at all** -- any string became a
real `UsermodTarget`, board typos included, confirmed live by reading `usermod_targets()`
directly before writing the fix (`values = axis_overrides.get(port) or
list(default_axis_values(port))`, no existence check anywhere on that path). The loose,
not-tag-gated version closes that real gap without duplicating Phase C2's own larger job.

**The migration error is dedicated, not generic.** A literal `[esp32] boards = [...]` now
raises a specific message naming the new `[esp32.BOARD_NAME]` shape, the same courtesy
`check_usermod_family_table()` already gives other syntax migrations -- not the generic
"unknown key" `check_keys()` would otherwise raise for a key that schema no longer accepts.
An unrecognised board name (`[esp32.NOT_A_REAL_BOARD]`) is its own separate, specific error
naming `resources/build-platforms.toml`. A board's own inner table is checked against
`USERMOD_PORT_BASE` (`user-c-modules`/`manifest`/`extra-make-args`), the same per-node
option surface every other tree node already has.

**Real configs updated in the same commit**: this repo's own `cibuildmp.toml` (a commented
sketch, not live config, but one that should not teach the removed syntax) and every test
fixture using the old `[esp32] boards = [...]` shape (`tests/test_usermod_options.py`'s own
`test_multiple_boards_same_port`, `tests/test_usermod_orchestrate.py`'s own
`test_build_one_esp32_passes_board_through`). `examples/template/cibuildmp.toml` has no
esp32 content today, confirmed directly, so nothing there needed touching.

Five new tests: the dedicated `boards = [...]` migration error; an unrecognised board name
rejected; a board node accepting its own `extra-make-args`; a board node rejecting an
unknown option key (`archs`, valid for `unix`/`windows` but not a board's own per-target
override surface); `test_multiple_boards_same_port` rewritten onto two `[esp32.<name>]`
tables rather than deleted. Live-verified end to end, not just unit-tested: a real two-board
config (`[esp32.ESP32_GENERIC]`, `[esp32.ESP32_GENERIC_S3]` with its own `extra-make-args`)
resolves to exactly two targets with the right `axis_overrides`.

424 tests pass; ruff and pyright clean. B4.3 (`[[overrides]]` dual matching -- B2's own
tree-path-or-identifier `select` mode) is next.
