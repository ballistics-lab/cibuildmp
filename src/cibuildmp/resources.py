...
def bootlin_releases() -> dict[str, Any]:
    """`resources/bootlin.toml` -- every Bootlin toolchain release this
    project could pin, with the versions inside it and the URL + sha256 to
    fetch it by (**record 0084**). Generated by
    `bin/fetch_bootlin_metadata.py` out of Bootlin's own manifests.

    A catalogue, not a selection: it carries `bleeding-edge` beside
    `stable` on purpose, because "stable only" is an argued *policy* and a
    table that quietly omitted the rejected half could not be used to
    re-check the argument later.

    `name` (`x86-64--glibc--stable-2025.08-1`) is the row's identity and
    the key anything else refers to it by -- it is already what
    `BOOTLIN_RELEASE=` is in `docker/ppc64le_linux.Dockerfile` and
    `docker/manylinux_2_41_mipsel.Dockerfile`, it is the tarball's own
    single top-level directory, and so it is the path `relocate-sdk.sh`
    runs at and the obvious cache key.
    """
    return _load("bootlin.toml")
